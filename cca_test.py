...
class TestCCA(unittest.TestCase):
    ...
    def test_cca_ledoit(self):
        ...
        fwd_func = CorrelationLoss.forward
        start = time()
        corr, _, _ = fwd_func(None, H1, H2, None, True, True)  # using autograd
        corr.backward()
        print("autograd time taken", time() - start)
        H1_grad_auto = np.copy(H1.grad.data)
        H2_grad_auto = np.copy(H2.grad.data)
        ...
        start = time()
        corr, _, _ = CorrLoss(H1, H2, None, True, True)  # using my forward & backward
        corr.backward()
        print("my grad time taken", time() - start)
        H1_grad_my = np.copy(H1.grad.data)
        H2_grad_my = np.copy(H2.grad.data)
    ...
    def test_cca_ledoit_no_mu_grad(self):
        ...
        fwd_func = CorrelationLoss.forward
        start = time()
        corr, _, _ = fwd_func(None, H1, H2, None, True, False)  # using autograd
        corr.backward()
        print("autograd time taken", time() - start)
        H1_grad_auto = np.copy(H1.grad.data)
        H2_grad_auto = np.copy(H2.grad.data)
        ...
        start = time()
        corr, _, _ = CorrLoss(H1, H2, None, True, False)  # using my forward & backward
        corr.backward()
        print("my grad time taken", time() - start)
        H1_grad_my = np.copy(H1.grad.data)
        H2_grad_my = np.copy(H2.grad.data)
